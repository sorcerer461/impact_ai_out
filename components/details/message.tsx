...
export function ChatGPTVersionSelector({payload, key}:{
  payload:any,
  key: any
}) {

  const versions = ['gpt-4-turbo', 'gpt-3.5-turbo', 'gemini-1.5-flash', 'claude-3-haiku'];
  ...
  const handleVersionChange = async (version: string) => {
    ...
    const streamIterator = streamingFetch('/api/response',
        {
          method: 'POST',
          headers: {
...
export function BotMessage({ content, icon, payload, className }: {
  content?: string,
  ...
  return (
    <div className='px-4 rounded-lg mb-2'>
      <div className='group/root pl-14 relative response-block scroll-mt-32 rounded-md hover:bg-gray-50 dark:hover:bg-white/5 pb-2 pt-2 pr-2  min-h-[52px]'>
        ...
        <div>
          <div className='absolute -bottom-5 right-4 items-start justify-center gap-1 flex'>
              <ChatGPTVersionSelector payload={payload} key={botMessageId} />
          </div>
        </div>
        <div className='w-full'>
